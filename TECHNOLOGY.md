...
| First CLI/profile slice | Python around the selected backend, with small shell fragments only where required for installation. | Avoid moving coordination/state logic into a growing shell script. A change of CLI language needs a demonstrated deployment or platform benefit. |
