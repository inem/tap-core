...
| `capture` | Optional `{request_body_paths}` policy for packs with `capture.read`. Paths are printable URL path substrings beginning with `/` (maximum 64, 512 characters each). Core scopes them to the pack's exact granted origins; `*` is forbidden because request bodies may contain credentials or full conversation history. The effective policy applies on the next profile start. |
...
| `page`: `browser-scripts-v1` | Ordered `uses` declarations, each with stable `id` and exact `version`; each resolves to a top-level `resources` provider. | Verify provider bytes, materialize the shared profile resource store, merge uses from all enabled packs per origin, and deduplicate identical resources. Core publishes an origin-scoped live plan; already-bootstrapped pages reconcile it and reload classic scripts when its revision changes. |
