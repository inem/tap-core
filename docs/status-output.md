...
- `--output terminal` explicitly selects the default view: runtime, routing, capture-writer, bridge-startup and managed-component sections, followed by cross-section and diagnostic alerts when warranted.
...
The classification and presentation choices are bundled inert JSON. Runtime, routing, capture, bridge, managed components and operator diagnostics own separate vocabulary, semantic and presentation fragments. The diagnostics fragment keeps the clear case silent, but adds a compact terminal warning when any inspection failed or a background pack has an integrity failure, including its stronger quarantined state. The warning points to `--output raw-json`, where exact error messages and affected pack names remain available. Capture distinguishes a ready current writer, an absent health record, a stale record, an unhealthy current writer, and incomplete inspection; readiness does not claim that traffic is currently flowing. Bridge first distinguishes absent, disabled, applied, unapplied, and unknown startup snapshots. Composition then refines `unapplied` with runtime state into inactive, drifted, blocked, or unverified before presentation. In ordinary `status`, `applied` confirms only configuration/PID agreement while preserving both `hub_liveness: not_checked` and `control_liveness: not_checked`; it does not claim that the Hub, handler, page, or end-to-end path is ready. The doctor projection can additionally show `control live` or `control unavailable` after its bounded probes.
...
The compact result and renderer now cover status runtime, routing, capture writer health, bridge startup state, the managed controller aggregate and its reader rows. `tap doctor` reuses this status summary and adds its existing backend, CA, proxy-rights and traffic checks plus next actions in a separate terminal view. Functional handler/page probes, measured reader backlog and loss, localization, arbitrary Unicode display width, external material loading and a standalone versioned doctor result remain open.
