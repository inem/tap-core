...
when handlers are configured (Hub path). Reader-only profiles without handlers
start the component controller without Bun/Hub; Bun is not probed in that mode.
