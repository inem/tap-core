...
This compact result is the stable input to optional status presentations. It carries only the runtime and routing observations needed by the first product slice. Every observation explicitly distinguishes a known value, an inspection failure, and a field that the supplied snapshot did not observe. The latter uses `{"knowledge":"unknown","reason":"not_observed","message":"observation was not supplied"}`.
