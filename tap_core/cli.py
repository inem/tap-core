...
def parser():
    ...
    for name in ("on", "off", "status", "doctor", "where", "uninstall"):
        ...
        if name == "status":
            command.add_argument("--output", choices=("json", "semantic-json", "terminal"),
                                 default="json")
            command.add_argument("--width", type=int, default=80,
                                 help="terminal projection width; optional meanings record omissions")
            command.add_argument("--theme", choices=("unicode", "ascii"), default="unicode")
...
def main(argv=None):
    ...
    try:
        ...
        if args.command in ("status", "doctor", "where"):
            ...
            if args.command == "status" and args.output == "semantic-json":
                from .status_projection import public_status_result
                print(json.dumps(public_status_result(result), indent=2))
            elif args.command == "status" and args.output == "terminal":
                from .status_projection import status_terminal
                use_color = args.color == "always" or (args.color == "auto" and sys.stdout.isatty())
                print(status_terminal(result, width=args.width, theme=args.theme, color=use_color))
            else:
